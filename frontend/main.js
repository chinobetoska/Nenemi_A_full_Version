...
async function obtenerDestinos() {
    ...
    try {
        ...
        datos.forEach(lugar => {
            const card = `
                <article class="destino-card" id="card-${escapeHTML(lugar._id)}">
                    <img src="${escapeHTML(lugar.foto)}" alt="${escapeHTML(lugar.nombre)}">
                    <div class="destino-info">
                        <h3>${escapeHTML(lugar.nombre)}</h3>
                        <p><strong>${escapeHTML(lugar.estado)}</strong></p>
...
function gestionarEstadoSesion() {
    ...
    if (nombreUsuario && userInfoDiv) {
        // Usar textContent para evitar XSS con el nombre del usuario
        const p = document.createElement('p');
        p.className = 'user-nombre-resaltado';
